            ...
            const esp_rmaker_param_val_t val, void *priv_data, esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (strcmp(esp_rmaker_param_get_name(param), ESP_RMAKER_DEF_POWER_NAME) == 0) {
        ...
        // Determine which door this is for and trigger pulse if ON
        ...
        // Don't update the parameter back to RainMaker - let it return to OFF automatically
...
void app_main()
{
    // CRITICAL: Set GPIO pins HIGH immediately to prevent relay activation during startup
    ...
    esp_rmaker_node_t *node = esp_rmaker_node_init(&rainmaker_cfg, "ESP RainMaker Device", "Garage Door Opener");
    if (!node) {
     ...
     * parameter.
     */
    esp_rmaker_device_add_param(door1_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Garage Door 1"));
    esp_rmaker_device_add_param(door2_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Garage Door 2"));

    /* Add the standard power parameter (type: esp.param.power), which adds a boolean param
     * with a toggle switch ui-type.
     */
    esp_rmaker_param_t *power_param1 = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, false);
    esp_rmaker_device_add_param(door1_device, power_param1);
    
    esp_rmaker_param_t *power_param2 = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, false);
    esp_rmaker_device_add_param(door2_device, power_param2);

    /* Assign the power parameter as the primary, so that it can be controlled from the
     * home screen of the phone apps.
     */
    esp_rmaker_device_assign_primary_param(door1_device, power_param1);
    esp_rmaker_device_assign_primary_param(door2_device, power_param2);

    /* Add this switch device to the node */
    esp_rmaker_node_add_device(node, door1_device);
    esp_rmaker_node_add_device(node, door2_device);
