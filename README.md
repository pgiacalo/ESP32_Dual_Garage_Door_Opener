...
3. Build: `idf.py fullclean && idf.py build`
4. Flash: `idf.py flash monitor`

### Requirements
- **ESP-IDF v5.5+** (tested with v5.5.1)
- **CMake 3.10+** (updated for compatibility)
- **Python 3.12** (recommended)
- **ESP32-C6** development board (primary target)

### Recent Updates ✅
- Fixed all CMake deprecation warnings
- Removed deprecated Kconfig symbols
- Updated managed components for compatibility
- Clean build output with no concerning warnings
...
# Quick build (if no config changes)
idf.py build
```
